...
public class GitRepositoryTest extends RepositoryTestBase {
	...
	private static Path commitTemporaryFile(DefaultExecutor executor, Path repository) throws Exception {
		...
		CommandLine commandAdd = new CommandLine("git").addArgument("add").addArgument(file.getFileName().toString());
		int resultAdd = executor.execute(commandAdd);
		assertEquals(0, resultAdd);
		CommandLine commandCommit = new CommandLine("git").addArgument("commit").addArgument("--message=message");
	...
	public static void prepareRepository() throws Exception {
		...
		CommandLine command = new CommandLine("git").addArgument("init", false);
		...
		repositoryObject = new GitRepository(repositoryPath.toAbsolutePath().toString(), temporaryPath);
